...
const main = async () => {
    ...
    const withdrawTx = StakeProgram.withdraw({
        ...
        lamports: stakedBalance
    })
    const withdrawTxId = await sendAndConfirmTransaction(connection, withdrawTx, [wallet])
    stakedBalance = await connection.getBalance(stakeAccount.publicKey)
    console.log(`Withdraw Tx id: ${withdrawTxId}, Staked account balance: ${stakedBalance / LAMPORTS_PER_SOL}`)
}
const runMain = async () => {
    try {
        await main()
